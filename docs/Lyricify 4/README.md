...
- **语言：** Lyricify 显示语言及 Spotify 内容语言。与 Spotify 的设置保持一致时可以获得最佳 Media Session 体验
- **中文翻译：** 是否在歌词有中文翻译时显示
...
## Lyricify 4 主界面
如果您正在 Spotify 播放音乐，那么不出意外的话该曲目的歌词会显示在 Lyricify 的 `歌词` 界面。  
...
#### Lyricify 4 获取歌词的过程（进阶）
  ...
  4. 特殊情况：如果步骤 1 或步骤 2 中获得的歌词的来源不是用户的首选来源，则会先在首选来源中搜索，在搜索不到的情况下，才会使用步骤 1 或步骤 2 中获得的歌词

### 歌词标记及导入
Lyricify 4 提供 `歌词标记` 功能，您可以把自动搜索的正确歌词在服务器上标记为正确，这样做有许多好处。
- **提高歌词获取效率：** 标记后 Lyricify 将直接从 Lyricify 服务器获取歌词，而不需要再进行自动搜索的过程，自动搜索耗时约 `0.5-2s`
...
1. 点击 `歌词` 界面中的 `手动导入 (打开曲目管理)`，进入 `曲目管理` 窗口。您还可以通过点击左下角歌名，在打开的菜单中点击 `打开曲目管理`。如果您正在 Apple Music 歌词 界面，则可以通过点击 `···` 圆形按钮，再点击 `打开曲目管理`。
...
5. 如果您想为中西文设置不同字体，可以自行了解 `字体回退机制`，Lyricify 4 自带的 `字体选择工具` 支持 `字体回落 (Font Fallback)`。
...
[这里有 Lyricify 的各语言原文文件](/i18n/Lyricify%204/README.md)
