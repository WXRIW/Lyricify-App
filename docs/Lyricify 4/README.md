...
**注意：**  
1. 并非所有字体都支持这种修改方式。
2. 如果有不理解的地方，您可以自行搜索和学习 `Json 文件` 的相关操作。
...
4. 如果 Spotify 里没有这首歌的歌词，而 酷狗音乐 中有这首歌的歌词，则可以通过点击 `获取酷狗歌词` 按钮，自动搜索并获取酷狗音乐歌词。
5. 如果没有获取到歌词，则需要在打开的窗口中粘贴歌词（请粘贴含时间轴的歌词文件！目前支持 LRC 和 QRC）
